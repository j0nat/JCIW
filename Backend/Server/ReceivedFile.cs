...
{    /*
    ...
    */
    class ReceivedFile
    {
        public delegate void ReceivedFileIdentifier(ReceivedFile file);
        public event ReceivedFileIdentifier ReceivedFileFinishedEvent;
        ...
        public void AddData(long dataStart, int bufferStart, int bufferLength, byte[] buffer)
        {
            lock (SyncRoot)
            {
                ...
                if (ReceivedBytes == SizeBytes)
                {
                    ReceivedFileFinishedEvent.Invoke(this);
                }
            }
        }
